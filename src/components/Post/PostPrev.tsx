...
import { faEllipsisVertical, faHeart, faShare } from "@fortawesome/free-solid-svg-icons";
...
const PostPrev = ({isBig = false}: PostPrevProps) => {
  ...
  return (
    <div className={classNames("main-border flex flex-row mb-8", {"flex-col max-w-[400px] lg:w-1/2  lg:ml-5 odd:ml-0" : isBig, "last-of-type:mb-0": !isBig})}>
      <a href="">
        <img className={classNames("w-64 h-80 object-cover", {"w-full h-auto" : isBig})} src={img} alt="" />
      </a>
      <div className={classNames("flex flex-col h-full w-80 px-7 pt-6 items-start", {"w-full": isBig})}>
        {isBig ? <PostReadInfoShare /> : <PostReadInfo/>}
        <a href="" className={classNames("post-prev-text", {"pb-10": isBig})}>
        ...
        <div className={classNames("flex flex-row justify-between border-t border-border-secondery-color w-full pt-4 pb-5", {"hidden": isBig})}>
